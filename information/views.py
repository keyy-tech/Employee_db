...
from .models import Task, Announcement, Notification
...
def update_task(request, id):
    ...
    if task.status in ["Complete", "Not Complete"]:
        messages.error(
            request,
            "Task cannot be updated because it is marked as complete or not complete",
        )
        return redirect("task_list")
...
def delete_task(request, task_id):
    ...
    if task.status in ["Complete", "Not Complete"]:
        messages.error(
            request,
            "Task cannot be deleted because it is marked as complete or not complete",
        )
        return redirect("task_list")
...
def task_list(request):
    if request.user.role == "HOD":
        ...
        tasks = Task.objects.filter(employee__department=department)
    else:
...
def complete_task(request, task_id):
    ...
    if task.status == "Complete":
        ...
        messages.error(
            request,
            "Task cannot be marked as complete because it is marked as not complete",
        )
    else:
...
def incomplete_task(request, task_id):
    ...
    if task.status == "Not Complete":
        ...
        messages.error(
            request,
            "Task cannot be marked as not complete because it is marked as complete",
        )
    else:
...
def delete_announcement(request, id):
    ...
    if announcement.created_by != request.user:
        messages.error(
            request, "You do not have permission to delete this announcement."
        )
        return redirect("announcement_list")
...
def update_announcement(request, id):
    ...
    if announcement.created_by != request.user:
        messages.error(
            request, "You do not have permission to update this announcement."
        )
        return redirect("announcement_list")
...
def user_notifications(request):
    ...
    return render(
        request, "information/notifications.html", {"notifications": notifications}
    )
